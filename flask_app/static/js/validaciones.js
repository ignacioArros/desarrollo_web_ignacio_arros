...
function revisaCheck(checkbox) {
    const input = document.getElementById(checkbox.name);
    if (checkbox.checked) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    regionSelect.addEventListener("change", (e) => {
      ...
      const selectedRegion = regionesYComunas.regiones.find(r => r.numero === regionId);
    ...
    // Mostrar input extra si se selecciona tema "otro"
    ...
    // Validación personalizada + confirmación
    ...
    const seleccionadas = redes.filter(r => document.querySelector(`input[name='${r}']`).checked);
